...
#include <fcntl.h>   // For open(), O_RDONLY, and O_NONBLOCK
...
bool initializeSenseHat() {
  joystick = open("/dev/input/event7", O_RDONLY);
    fprintf(stdout, "OPEN joystick: %d\n", joystick);
  if (joystick < 0) {
    return false;
  }
  return true;
...
void handle_joystick_event(struct js_event *e) {
    switch (e->type & ~JS_EVENT_INIT) {
    case JS_EVENT_BUTTON:
        if (e->value) {
            fprintf(stdout, "Button %d pressed\n", e->number);
        } else {
            fprintf(stdout, "Button %d released\n", e->number);
        }
        break;
    case JS_EVENT_AXIS:
        fprintf(stdout, "Axis %d moved to value %d\n", e->number, e->value);
        break;
    }
}
...
// fprintf(stdout, "joystick: %d\n", joystick);
  ret = read (joystick, &e, sizeof(e));
    if (ret == sizeof(e)) {
        handle_joystick_event(&e);
    } else {
        fprintf(stderr, "Err: read: %d, errno: %d\n", ret, errno);
    }
...
int main(int argc, char **argv) {
  ...
  resetPlayfield();
  fprintf(stdout, "gameover call......\n");
  // Start with gameOver
  gameOver();

    fprintf(stdout, "Will open joystick\n");
